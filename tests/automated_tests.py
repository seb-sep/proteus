...
from test_modules import SimpleModule, EmbeddingModule, SimpleTransformer, TestModule
...
class TestProteus(unittest.TestCase):
    ...
    def compile_model_test(self, model: nn.Module, test_input: torch.Tensor):
        ...
        self.assertTrue(
            torch.allclose(
                test_out, coerce_mx_to_torch(compiled_out), rtol=1e-4, atol=1e-4
            ),
            f"Compiled {model.__qualname__} model output does not match original output",
        )
